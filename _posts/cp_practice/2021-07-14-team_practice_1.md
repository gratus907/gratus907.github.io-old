...
## Phase 0 : Start 
리얼리티를 위해 문제 순서를 Random shuffle 하고, `gyuni-gratus907-dlwocks31` 순서로 3 3 2씩 나눠 읽기로 랬습니다. 
...
- 다만, 이때 $O(n^2)$ 개의 pair를 확인해야 하므로, $O(n^2 \log n)$ 시간이 걸리는데, 이는 도저히 답이 없는 복잡도입니다.
...
- 그러나, 이 방법은 잘 생각해보면 맵을 합치는 데 $O(n \log n)$ 시간까지 걸리기 때문에, $O(n^2 \log n)$ 시간이 걸립니다.
